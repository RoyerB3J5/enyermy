    ...
    <section className="grid grid-cols-1 lg:grid-cols-2 justify-center items-center w-full">
      ...
      <div
        className={`w-full py-12  h-auto ${changeAspectRatio ? "lg:aspect-720/500" : "lg:aspect-720/660"} ${
          isParity
            ? "lg:order-2 bg-white lg:py-3 xl:py-0 xl:pr-[max(2rem,calc((100vw-1280px)/2))] "
            : "lg:order-1 bg-accent lg:py-0 xl:pl-[max(2rem,calc((100vw-1280px)/2))] "
        }  flex items-center`}
      >
        <div
          className={`w-full px-4 xl:px-0 lg:max-w-160 flex ${isParity ? "justify-end md:pl-6 fade-left" : "justify-start fade-right"}`}
        >
          <div
          ...
          >
            ...
            {content.media && content.linkContent && (
              <ButtonLink content={content.linkContent} openInNewTab />
            )}
